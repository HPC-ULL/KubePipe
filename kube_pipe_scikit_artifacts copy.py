...
"""    
                        ...
                        'inputs' : {
                            'artifacts':
                               [
                                    ...
                                    {"name" : f"iny{i}{pipeId}", "path" :  "/tmp/y",    "s3":    { "key" :  f"{BUCKET_PATH}/{self.id}/tmp/y" }},
                                    {"name" : f"inX{i}{pipeId}", "path" :  "/tmp/X"}
            ...
            if(i == 0):
                template["inputs"]["artifacts"][-1]["s3"] =  { "key" :  f"{BUCKET_PATH}/{self.id}/tmp/X" }                        
                
            else:
                template["arguments"] = {"artifacts" : {
                    "name" : f"inX{i}{pipeId}",
                    "from" : "{{steps.generate-artifact.outputs.artifacts." + f"output{pipeId}{i-1}" + "}}"
                }}
            ...
            #Transformer
                ...
                template["outputs"]["artifacts"].append({"name" : f"outX{pipeId}{i}", "path" : "/tmp/X", "archive" : {"none" : {}}, "s3": { "key" : f"{BUCKET_PATH}/{self.id}/tmp/X{pipeId}"}})
